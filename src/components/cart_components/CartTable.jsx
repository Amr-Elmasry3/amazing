              ...
              {products.map((item) => {
                return (
                  <TableRow key={item.id}>
                    ...
                    <TableCell
                      align="center"
                      className="text-[14px]! font-[600]! text-light-navy! border-b border-l border-solid border-border-one flex justify-center"
                    >
                      <div
                        className="flex items-center justify-between bg-section rounded-one"
                        style={{ padding: "5px" }}
                      >
                        <div
                          className="decrease-icon bg-light-navy rounded-one cursor-pointer"
                          onClick={() => {
                            handleDecreaseCount(item);
                          }}
                        >
                          <FaMinus className="icon text-[16px] text-white" />
                        </div>

                        <span className="count">{item.count}</span>

                        <div
                          className="increase-icon bg-light-navy rounded-one cursor-pointer"
                          onClick={() => {
                            handleIncreaseCount(item);
                          }}
                        >
                          <FaPlus className="icon text-[16px] text-white" />
                        </div>
                      </div>
                    </TableCell>
